...
public static void main(String[] args){
    ...
    order1.items.add(item1.getName());
    order1.items.add(item4.getName());
    order1.total = item1.getPrice() + item4.getPrice();
    order1.ready = true;
    CafeOrder order2 = new CafeOrder("Jimmy", 0, false, new ArrayList<String>());
    order2.items.add(item2.getName());
    order2.items.add(item2.getName());
    order2.items.add(item3.getName());
